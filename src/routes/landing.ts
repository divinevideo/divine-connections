...
export function renderLandingPage(env: Env, origin: string): string {
  ...
  const ytTableRow = hasYouTubeProof ? '<tr><td><code>youtube</code></td><td>Channel ID (<code>UCxxxx</code>) or handle (<code>@user</code>)</td><td>Video ID (11 chars)</td><td>Yes</td></tr>' : ''
  const extraLookupPlatforms = ",'youtube','tiktok'"
  const choosePlatforms = 'Choose X, GitHub, Bluesky, Mastodon, Telegram, Discord, YouTube, TikTok, or Instagram.'
  const noPostingPlatforms = 'No posting required for X, Instagram, TikTok, and YouTube.'
  const OAUTH_OPTION_LABELS = { x: 'Twitter / X', instagram: 'Instagram', tiktok: 'TikTok', youtube: 'YouTube' }
  const oauthPlatformOptions = (['x', 'instagram', 'tiktok', 'youtube'] as const)
  ...
  <div class="container">
    ...
    <section id="platforms-api">
      ...
      <table>
        ...
        ${ytTableRow}
        <tr><td><code>tiktok</code></td><td>Username (without @)</td><td>Video ID (numeric)</td><td>Yes</td></tr>
      </table>
    </section>
