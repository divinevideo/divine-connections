// ABOUTME: Public proof-post verification routes with the legacy verifier's response
// ABOUTME: contract; the D1/KV/rate-limit/upstream flow lives in services/verify.ts.
import { Hono } from 'hono'
import type { Env } from '../types'
import { isValidIdentity, isValidProof } from '../verify/validation'
import { hexToNpub } from '../utils/npub'
import { normalizeVerifyPlatform, verifySingleClaim } from '../services/verify'
import type { VerifyClaim, VerifyResult } from '../services/verify'
...
// The merged platform set: legacy proof-post platforms plus the OAuth-backed
// providers and the canonical 'x' spelling (legacy 'twitter' stays accepted).
const VALID_PLATFORM_LIST = 'github, twitter, x, instagram, mastodon, telegram, bluesky, discord, youtube, tiktok'

function validateClaim(claim: VerifyClaim, index: number): { index: number; error: string } | null {
...
verify.post('/', async (c) => {
  ...
  const results = await Promise.all(
    body.claims.map((claim) => verifySingleClaim(c.env, claim, { ip: clientIp(c) })),
  )
...
verify.post('/single', async (c) => {
  ...
  const result = await verifySingleClaim(c.env, claim, { ip: clientIp(c) })
...
verify.get('/:platform/*', async (c) => {
  ...
  if (!platform || !normalizeVerifyPlatform(platform)) {
  ...
  const claim: VerifyClaim = { pubkey, platform, identity, proof }
  const result = await verifySingleClaim(c.env, claim, { ip: clientIp(c) })
  ...
  if (accept.includes('text/html') && !accept.includes('application/json')) {
    const npub = hexToNpub(pubkey.toLowerCase())
...
function proofUrl(platform: string, identity: string, proof: string): string | null {
  switch (platform) {
    ...
    case 'twitter':
    case 'x': return `https://x.com/${identity}/status/${proof}`
...
function platformIcon(platform: string): string {
  switch (platform) {
    ...
    case 'twitter':
    case 'x': return '<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>'
...
<body>
  ...
  (function() {
    ...
    async function init() {
      ...
      var supportedPlatforms = ['github','twitter','x','mastodon','telegram','bluesky','discord','youtube','tiktok'];
      var claims = [];
