// ABOUTME: NIP-05 verification route: fetches the domain's well-known nostr.json
// ABOUTME: and checks the name's registered pubkey. Keeps route-level IP limiting.
import { Hono } from 'hono'
import type { Env } from '../types'
import type { CachedResult } from '../utils/cache'
import { validateNip05Name } from '../verify/validation'
import { nip05CacheKey, getCached, putCached } from '../utils/cache'
import { checkRateLimit, RATE_LIMITS } from '../utils/rate-limit'

const nip05 = new Hono<{ Bindings: Env }>()
...
nip05.get('/verify', async (c) => {
  ...
  // IP rate limit
  const ipLimit = await checkRateLimit(c.env.CACHE_KV, RATE_LIMITS.ip, clientIp)
  if (!ipLimit.allowed) {
