// ABOUTME: Discord proof-post verifier: resolves a message and checks its author
// ABOUTME: against the claimed username and its content for the claimant's npub.
...
export class DiscordVerifier implements PlatformVerifier {
  ...
  async verify(
    ...
    // endpoint returns no `inviter` for permanent or vanity invites, so the only
    // thing an invite establishes is that somebody put an npub in a server's name
    // or description — with no link to the claimed username. Accepting it let
    // anyone claim any handle, so invites are refused outright.
    return {
      verified: false,
